                           ...
                           uint8_t *h0, uint8_t *h1, uint8_t *h2, uint8_t *h3)
{
    ...
    alignas(16) unsigned char sh0[64];
    alignas(16) unsigned char sh1[64];
    alignas(16) unsigned char sh2[64];
    alignas(16) unsigned char sh3[64];
...
uint8_t Secp256K1::GetByte(std::string &str, int idx)
{
    ...
    if (sscanf(tmp, "%X", &val) != 1) {
        ...
        throw std::runtime_error("ParsePublicKeyHex invalid hexadecimal digit");
    }
...
Point Secp256K1::ParsePublicKeyHex(std::string str, bool &isCompressed)
{
    ...
    if (str.length() < 2) {
        ...
        throw std::runtime_error("ParsePublicKeyHex invalid public key length");
    }
    ...
    case 0x02:
        if (str.length() != 66) {
            ...
            throw std::runtime_error("ParsePublicKeyHex invalid compressed public key length");
        }
        for (int i = 0; i < 32; i++)
    ...
    case 0x04:
        if (str.length() != 130) {
            ...
            throw std::runtime_error("ParsePublicKeyHex invalid uncompressed public key length");
        }
        for (int i = 0; i < 32; i++)
            ret.x.SetByte(31 - i, GetByte(str, i + 1));
        for (int i = 0; i < 32; i++)
    ...
    default:
        ...
        throw std::runtime_error("ParsePublicKeyHex invalid public key prefix");
    }
    ...
    if (!EC(ret)) {
        ...
        throw std::runtime_error("ParsePublicKeyHex point is not on elliptic curve");
    }
