    ...
    if (!CryptAcquireContext(
        ...
        if (GetLastError() == NTE_BAD_KEYSET) {
                if (!CryptAcquireContext(
                        ...
                        PROV_RSA_FULL,
                        CRYPT_NEWKEYSET)) {
                fprintf(stderr, "CryptAcquireContext(): Could not create a new key container.\n");
                throw std::runtime_error("CryptAcquireContext new key container failed");
            }
        } else {
            fprintf(stderr, "CryptAcquireContext(): A cryptographic service handle could not be acquired.\n");
            throw std::runtime_error("CryptAcquireContext failed");
        }
    }

    if (!CryptGenRandom(hCryptProv, size, buff)) {
        fprintf(stderr, "CryptGenRandom(): Error during random sequence acquisition.\n");
