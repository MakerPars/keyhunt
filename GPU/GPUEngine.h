...
private:
	...
	bool callKernel();
	bool callKernel2();
	bool callKernel(uint32_t* targetOutputBuffer);
	bool callKernel2(uint32_t* targetOutputBuffer);

	int CheckBinary(const uint8_t* hash);

#ifdef WITHGPU
	void ConfigureCacheForArch(const cudaDeviceProp& prop);
	int ComputeOptimalGridBlocks(const cudaDeviceProp& prop, int requestedGroups);
	size_t ComputeOptimalStackSize(const cudaDeviceProp& prop);
#endif
	...
	uint64_t* inputKey2;
	uint64_t* inputKeyPinned2;

	uint32_t* outputBuffer;
	uint32_t* outputBufferPinned;
	uint32_t* outputBuffer2;
	uint32_t* outputBufferPinned2;

#ifdef WITHGPU
	cudaStream_t computeStream;
	cudaStream_t transferStream;
	cudaEvent_t kernelDoneEvent;
	cudaEvent_t transferDoneEvent;
#endif
	bool pipelinePrimed;
